...
use crate::cube::*;
use crate::defs::*;
use crate::solver::df_search::{dfs_iter, CancelToken};
use crate::solver::lookup_table::{
    DepthEstimate, InMemoryIndexTable, InMemoryNissIndexTable, NissDepthEstimate,
};
...
impl DefaultStepOptions {
    pub fn new(
        min_moves: u8,
        max_moves: u8,
        absolute_min_moves: Option<u8>,
        absolute_max_moves: Option<u8>,
...
struct NissPruningTableHeuristic<'a, const HC_SIZE: usize, HC: Coord<HC_SIZE> + 'a>(
    &'a InMemoryNissIndexTable<HC_SIZE, HC>,
);
struct PruningTableHeuristic<'a, const HC_SIZE: usize, HC: Coord<HC_SIZE> + 'a>(
    &'a InMemoryIndexTable<HC_SIZE, HC>,
);
...
impl<const HC_SIZE: usize, HC: Coord<HC_SIZE>> Heuristic for PruningTableHeuristic<'_, HC_SIZE, HC>
where
    HC: for<'a> From<&'a Cube333>,
{
    fn heuristic(&self, cube: &Cube333, can_niss: bool) -> u8 {
...
impl<const HC_SIZE: usize, HC: Coord<HC_SIZE>> Heuristic
    for NissPruningTableHeuristic<'_, HC_SIZE, HC>
where
    HC: for<'a> From<&'a Cube333>,
{
    fn heuristic(&self, cube: &Cube333, can_niss: bool) -> u8 {
...
impl<'a, const HC_SIZE: usize, HC: Coord<HC_SIZE>, const PC_SIZE: usize, PC: Coord<PC_SIZE>>
    PreStepCheck for DefaultPruningTableStep<'a, HC_SIZE, HC, PC_SIZE, PC>
where
    PC: for<'b> From<&'b Cube333>,
{
...
impl<'a, const HC_SIZE: usize, HC: Coord<HC_SIZE>, const PC_SIZE: usize, PC: Coord<PC_SIZE>>
    PostStepCheck for DefaultPruningTableStep<'a, HC_SIZE, HC, PC_SIZE, PC>
where
    PC: for<'b> From<&'b Cube333>,
{
    fn is_solution_admissible(&self, cube: &Cube333, alg: &Algorithm) -> bool {
...
impl<'a, const HC_SIZE: usize, HC: Coord<HC_SIZE>, const PC_SIZE: usize, PC: Coord<PC_SIZE>>
    StepVariant for DefaultPruningTableStep<'a, HC_SIZE, HC, PC_SIZE, PC>
where
    PC: for<'b> From<&'b Cube333>,
    HC: for<'b> From<&'b Cube333>,
{
...
impl<'a, const HC_SIZE: usize, HC: Coord<HC_SIZE>, const PC_SIZE: usize, PC: Coord<PC_SIZE>>
    DefaultPruningTableStep<'a, HC_SIZE, HC, PC_SIZE, PC>
where
    PC: for<'b> From<&'b Cube333>,
    HC: for<'b> From<&'b Cube333>,
{
...
pub fn next_step<'a, 'b, IN: Iterator<Item = Solution> + 'a>(
    algs: IN,
    step: &'a Step<'b>,
    search_opts: DefaultStepOptions,
    cube: Cube333,
    ...
    stream::iterated_dfs(algs, cancel_token, move |solution, depth, cancel_token| {
        ...
        let result: Box<dyn Iterator<Item = Solution>> = if depth < search_opts.min_moves
            || depth > search_opts.max_moves
            || search_opts
                .absolute_min_moves
                .map(|m| m > absolute_target_length)
                .unwrap_or(false)
            || search_opts
                .absolute_max_moves
                .map(|m| m < absolute_target_length)
                .unwrap_or(false)
        {
            ...
            cube.apply_alg(&alg);
            let stage_opts = DefaultStepOptions::new(
                depth,
                depth,
                None,
                None,
                search_opts.niss_type,
                search_opts.step_limit,
            );
