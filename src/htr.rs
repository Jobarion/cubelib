...
pub fn htr_any<'a, C: 'a + Debug>(table: &'a PruningTable<{coord::HTRDRUD_SIZE}, HTRDRUDCoord>) -> Step<'a, 4, 6, C> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C>  {
    Step::new(vec![
...
pub fn htr<'a, C: 'a + Debug, const DRA: usize>(table: &'a PruningTable<{coord::HTRDRUD_SIZE}, HTRDRUDCoord>, dr_axis: [Axis; DRA]) -> Step<'a, 4, 6, C> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C>  {
    let step_variants = dr_axis.into_iter()
        .flat_map(move |x| {
...
impl <'a, C: Debug> StepVariant<4, 6, C> for HTRDRUDStageTable<'a> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C> {
    ...
    fn heuristic(&self, cube: &C) -> u8 {
        ...
        if self.table.get(coord).is_none() {
            println!("{:?}", cube);
        }
...
impl <'a, C> StepVariant<4, 6, C> for HTRDRFBStageTable<'a> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C> {
    fn move_set(&self) -> &'a MoveSet<4, 6> {
        self.move_set
    }
...
impl <'a, C> StepVariant<4, 6, C> for HTRDRLRStageTable<'a> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C> {
    fn move_set(&self) -> &'a MoveSet<4, 6> {
        self.move_set
    }
...
impl <'a, C> IsReadyForStep<C> for HTRDRUDStageTable<'a> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C> {
    fn is_cube_ready(&self, cube: &C) -> bool {
        DRUDEOFBCoord::from(cube).val() == 0
    }
}

impl <'a, C> IsReadyForStep<C> for HTRDRFBStageTable<'a> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C> {
    fn is_cube_ready(&self, cube: &C) -> bool {
        DRUDEOFBCoord::from(cube).val() == 0
    }
}

impl <'a, C> IsReadyForStep<C> for HTRDRLRStageTable<'a> where HTRDRUDCoord: for<'x> From<&'x C>, DRUDEOFBCoord: for<'x> From<&'x C> {
    fn is_cube_ready(&self, cube: &C) -> bool {
        DRUDEOFBCoord::from(cube).val() == 0
    }
}
