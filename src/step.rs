...
pub struct Step<'a, const SC_SIZE: usize, const AUX_SIZE: usize, C> {
    step_variants: Vec<Box<dyn StepVariant<SC_SIZE, AUX_SIZE, C> + 'a>>
}

impl <'a, const SC_SIZE: usize, const AUX_SIZE: usize, C> Step<'a, SC_SIZE, AUX_SIZE, C> {
    pub fn new(step_variants: Vec<Box<dyn StepVariant<SC_SIZE, AUX_SIZE, C> + 'a>>) -> Self {
        Step {
            step_variants
        }
    }
...
>(
    ...
    next_step(vec![Algorithm::new()].into_iter(), step, search_opts, cube)
}
