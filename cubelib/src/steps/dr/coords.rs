...
mod avx2 {
    use std::arch::x86_64::{
        __m128i, _mm_add_epi8, _mm_and_si128, _mm_cmpeq_epi8, _mm_extract_epi16, _mm_hadd_epi16,
        _mm_hadd_epi32, _mm_mullo_epi16, _mm_or_si128, _mm_sad_epu8, _mm_set1_epi32, _mm_set1_epi8,
        _mm_setr_epi32, _mm_setr_epi8, _mm_shuffle_epi32, _mm_shuffle_epi8, _mm_srli_epi32,
        _mm_sub_epi8,
    };
...
mod neon {
    use std::arch::aarch64::{
        uint16x8_t, uint8x16_t, vaddq_u8, vaddvq_u16, vand_u8, vandq_u8, vceqq_u8, vcombine_u8,
        vdup_n_u8, vdupq_n_u8, vmulq_u16, vorrq_u8, vqtbl1q_u8, vreinterpretq_u16_u8, vshrq_n_u8,
        vsubq_u8, vzip1q_u8, vzip2q_u8,
    };
    ...
    const UD_SLICE_BINOM_2: uint8x16_t = unsafe {
        C16 {
            a_u8: UD_SLICE_BINOM_2_ARR,
        }
        .a
    ...
    const CO_MUL: uint16x8_t = unsafe {
        C16 {
            a_u16: [1, 3, 9, 27, 81, 243, 729, 0],
        }
        .a_16
    ...
    pub(crate) unsafe fn unsafe_from_udslice_unsorted_coord(
        ...
        let coord = unsafe {
            let slice_edges = vshrq_n_u8::<6>(vandq_u8(value.0, vdupq_n_u8(0b01000000)));
            //Our edge order is
            // UB UR UF UL FR FL BR BL DF DR DB DL
            ...
            let slice_edges = vqtbl1q_u8(
                slice_edges,
                C16 {
                    a_i8: [1, 2, 3, 0, 9, 8, 11, 10, 4, 5, 7, 6, -1, -1, -1, -1],
                }
                .a,
            );
            ...
            let edge_sums = vaddq_u8(
                slice_edges,
                vqtbl1q_u8(
                    slice_edges,
                    C16 {
                        a_i8: [-1, 0, 1, 2, -1, 4, 5, 6, -1, 8, 9, 10, -1, -1, -1, -1],
                    }
                    .a,
                ),
            );
            ...
            let edge_sums = vaddq_u8(
                ...
                vqtbl1q_u8(
                    edge_sums,
                    C16 {
                        a_i8: [-1, -1, 0, 1, -1, -1, 4, 5, -1, -1, 8, 9, -1, -1, -1, -1],
                    }
                    .a,
                ),
            );
            ...
            let edge_sums = vaddq_u8(
                ...
                vqtbl1q_u8(
                    edge_sums,
                    C16 {
                        a_i8: [-1, -1, -1, -1, 3, 3, 3, 3, -1, -1, -1, -1, -1, -1, -1, -1],
                    }
                    .a,
                ),
            );
            ...
            let edge_sums = vaddq_u8(
                ...
                vqtbl1q_u8(
                    edge_sums,
                    C16 {
                        a_i8: [-1, -1, -1, -1, -1, -1, -1, -1, 7, 7, 7, 7, -1, -1, -1, -1],
                    }
                    .a,
                ),
            );
            ...
            let lut_index = vandq_u8(
                vsubq_u8(non_slice_edge_sums, vdupq_n_u8(1)),
                vdupq_n_u8(0b10001111_u8),
            );

            let lut_index = vaddq_u8(
                lut_index,
                C16 {
                    a_u8: [0, 4, 8, 12, 0, 4, 8, 12, 0, 4, 8, 12, 0, 0, 0, 0],
                }
                .a,
            );
            ...
            let hsum_upper =
                vaddvq_u16(vreinterpretq_u16_u8(vzip2q_u8(binom891011, vdupq_n_u8(0))));
...
mod wasm32 {
    use std::arch::wasm32::{
        i32x4, i8x16, u16x8, u16x8_extract_lane, u16x8_mul, u32x4_shr, u32x4_shuffle, u8x16,
        u8x16_add, u8x16_eq, u8x16_sub, u8x16_swizzle, v128, v128_and, v128_or,
    };
    ...
    use crate::steps::dr::coords::{COUDCoord, UDSliceUnsortedCoord};
    use crate::wasm_util::{
        complete_hsum_epi16, hsum_narrow_epi32, hsum_wide_epi32, mm_sad_epu8, u8x16_set1,
    };
    ...
    const CO_SHUFFLE_8_TO_16: v128 = u8x16(
        0, 0xFF, 1, 0xFF, 2, 0xFF, 3, 0xFF, 4, 0xFF, 5, 0xFF, 6, 0xFF, 7, 0xFF,
    );
    ...
    pub(crate) fn from_cocoord(value: &CubeCornersOdd) -> COUDCoord {
        //Spread co data out into 16bit values to avoid overflow later
        let co_epi16 = v128_and(u8x16_swizzle(value.0, CO_SHUFFLE_8_TO_16), u8x16_set1(0b11));
        //Multiply with 3^0, 3^1, etc.
    ...
    #[inline]
    pub(crate) fn from_udslice_unsorted_coord(value: &EdgeCube333) -> UDSliceUnsortedCoord {
        let coord = {
            let slice_edges = u32x4_shr(v128_and(value.0, u8x16_set1(0b01000000)), 6);
            //Our edge order is
            // UB UR UF UL FR FL BR BL DF DR DB DL
            ...
            let hadd4567891011 = hsum_narrow_epi32(u8x16_swizzle(
                hadd,
            ...
            let hadd = v128_or(
                u32x4_shuffle::<4, 0, 1, 2>(hadd4567891011, i32x4(0, 0, 0, 0)),
                hadd0123,
            );

            let hadd = v128_and(hadd, non_slice_edge_mask);

            let lut_index = v128_and(u8x16_sub(hadd, u8x16_set1(1)), u8x16_set1(0b10001111));
