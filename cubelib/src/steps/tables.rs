...
use crate::solver::lookup_table::{
    DepthEstimate, InMemoryIndexTable, InMemoryNissIndexTable, TableError,
};
#[cfg(feature = "fs")]
...
#[cfg(feature = "333dr")]
use crate::steps::dr::dr_config::{DRPruningTable, DR_UD_EO_FB_MOVESET};
#[cfg(feature = "333eo")]
use crate::steps::eo::coords::EOCoordFB;
#[cfg(feature = "333eo")]
use crate::steps::eo::eo_config::{EOPruningTable, EO_FB_MOVESET};
#[cfg(feature = "333finish")]
use crate::steps::finish::coords::{FRUDFinishCoord, HTRFinishCoord, HTRLeaveSliceFinishCoord};
#[cfg(feature = "333finish")]
use crate::steps::finish::finish_config::{
    FRFinishPruningTable, HTRFinishPruningTable, HTRLeaveSliceFinishPruningTable,
    FRUD_FINISH_MOVESET, HTR_FINISH_MOVESET,
};
#[cfg(feature = "333fr")]
use crate::steps::fr::coords::{FRUDNoSliceCoord, FRUDWithSliceCoord};
#[cfg(feature = "333fr")]
use crate::steps::fr::fr_config::{FRLeaveSlicePruningTable, FRPruningTable, FR_UD_MOVESET};
#[cfg(feature = "333htr")]
use crate::steps::htr::coords::HTRDRUDCoord;
#[cfg(feature = "333htr")]
use crate::steps::htr::htr_config::{HTRPruningTable, HTRSubsetTable, HTR_DR_UD_MOVESET};
...
impl PruningTables333 {
    ...
    pub fn save(&self, key: &str) -> Result<(), TableError> {
        match key {
            #[cfg(feature = "333eo")]
            "eo" => {
                if let Some(tbl) = &self.eo {
                    tbl.save_to_disk("333", key)?
                }
            }
            #[cfg(feature = "333dr")]
            "dr" => {
                if let Some(tbl) = &self.dr {
                    tbl.save_to_disk("333", key)?
                }
            }
            #[cfg(feature = "333htr")]
            "htr" => {
                if let Some(tbl) = &self.htr {
                    tbl.save_to_disk("333", key)?;
                }
            }
            #[cfg(feature = "333htr")]
            "htr-subset" => {
                if let Some(tbl) = &self.htr_subset {
                    tbl.save_to_disk("333", format!("{key}").as_str())?;
                }
            }
            #[cfg(feature = "333fr")]
            "fr" => {
                if let Some(tbl) = &self.fr {
                    tbl.save_to_disk("333", key)?
                }
            }
            #[cfg(feature = "333fr")]
            "frls" => {
                if let Some(tbl) = &self.frls {
                    tbl.save_to_disk("333", key)?
                }
            }
            #[cfg(feature = "333finish")]
            "frfin" => {
                if let Some(tbl) = &self.fr_finish {
                    tbl.save_to_disk("333", key)?
                }
            }
            #[cfg(feature = "333finish")]
            "htrfin" => {
                if let Some(tbl) = &self.htr_finish {
                    tbl.save_to_disk("333", key)?
                }
            }
            #[cfg(feature = "333finish")]
            "htrlsfin" => {
                if let Some(tbl) = &self.htr_ls_finish {
                    tbl.save_to_disk("333", key)?
                }
            }
            _ => {}
    ...
    pub fn load(&mut self, key: &str) -> Result<(), TableError> {
        match key {
            ...
            "htr" => {
                ...
                self.htr_subset = Some(HTRSubsetTable::load_from_disk(
                    "333",
                    format!("{key}-subset").as_str(),
                )?)
            }
            #[cfg(feature = "333fr")]
    ...
    #[cfg(feature = "fs")]
    pub fn load_and_gen_normal<const C_SIZE: usize, C: Coord<C_SIZE>>(
        key: &str,
        val: &mut Option<InMemoryIndexTable<C_SIZE, C>>,
        gen_f: &dyn Fn() -> InMemoryIndexTable<C_SIZE, C>,
        load_f: &dyn Fn() -> Result<InMemoryIndexTable<C_SIZE, C>, TableError>,
    ) -> bool {
        if val.is_none() {
            let res = load_f();
            match res {
    ...
    #[cfg(feature = "fs")]
    pub fn load_and_gen_niss<const C_SIZE: usize, C: Coord<C_SIZE>>(
        key: &str,
        val: &mut Option<InMemoryNissIndexTable<C_SIZE, C>>,
        gen_f: &dyn Fn() -> InMemoryNissIndexTable<C_SIZE, C>,
        load_f: &dyn Fn() -> Result<InMemoryNissIndexTable<C_SIZE, C>, TableError>,
    ) -> bool {
        if val.is_none() {
            let res = load_f();
            match res {
    ...
    #[cfg(feature = "fs")]
    pub fn load_and_save_normal<const C_SIZE: usize, C: Coord<C_SIZE>>(
        &mut self,
        key: &str,
        mut_f: &dyn Fn(&mut Self) -> &mut Option<InMemoryIndexTable<C_SIZE, C>>,
        gen_f: &dyn Fn() -> InMemoryIndexTable<C_SIZE, C>,
        load_f: &dyn Fn() -> Result<InMemoryIndexTable<C_SIZE, C>, TableError>,
    ) -> bool {
    ...
    #[cfg(feature = "fs")]
    pub fn load_and_save_niss<const C_SIZE: usize, C: Coord<C_SIZE>>(
        &mut self,
        key: &str,
        mut_f: &dyn Fn(&mut Self) -> &mut Option<InMemoryNissIndexTable<C_SIZE, C>>,
        gen_f: &dyn Fn() -> InMemoryNissIndexTable<C_SIZE, C>,
        load_f: &dyn Fn() -> Result<InMemoryNissIndexTable<C_SIZE, C>, TableError>,
    ) -> bool {
    ...
    pub fn gen_eo(&mut self) {
        self.load_and_save_normal("eo", &|x| &mut x.eo, &gen_eo, &|| {
            EOPruningTable::load_from_disk("333", "eo")
        });
    }
    ...
    pub fn gen_dr(&mut self) {
        self.load_and_save_normal("dr", &|x| &mut x.dr, &gen_dr, &|| {
            DRPruningTable::load_from_disk("333", "dr")
        });
    }
    ...
    pub fn gen_htr(&mut self) {
        let new_table = self.load_and_save_niss("htr", &|x| &mut x.htr, &gen_htr, &|| {
            HTRPruningTable::load_from_disk("333", "htr")
        });
    ...
    pub fn gen_fr_leave_slice(&mut self) {
        self.load_and_save_normal("frls", &|x| &mut x.frls, &gen_fr_leave_slice, &|| {
            FRLeaveSlicePruningTable::load_from_disk("333", "frls")
        });
    }
    ...
    pub fn gen_fr(&mut self) {
        self.load_and_save_normal("fr", &|x| &mut x.fr, &gen_fr, &|| {
            FRPruningTable::load_from_disk("333", "fr")
        });
    }
    ...
    pub fn gen_fr_finish(&mut self) {
        self.load_and_save_normal("frfin", &|x| &mut x.fr_finish, &gen_fr_finish, &|| {
            FRFinishPruningTable::load_from_disk("333", "frfin")
        });
    }
    ...
    pub fn gen_htr_finish(&mut self) {
        self.load_and_save_normal("htrfin", &|x| &mut x.htr_finish, &gen_htr_finish, &|| {
            HTRFinishPruningTable::load_from_disk("333", "htrfin")
        });
    }
    ...
    pub fn gen_htr_leave_slice_finish(&mut self) {
        self.load_and_save_normal(
            "htrlsfin",
            &|x| &mut x.htr_ls_finish,
            &gen_htr_no_slice_finish,
            &|| HTRLeaveSliceFinishPruningTable::load_from_disk("333", "htrlsfin"),
        );
    }
...
fn gen_eo() -> EOPruningTable {
    ...
    let table = lookup_table::generate(
        &EO_FB_MOVESET,
