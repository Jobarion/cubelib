...
use crate::steps::finish::coords::{
    DRFinishCoord, DRLeaveSliceFinishCoord, FRUDFinishCoord, HTRFinishCoord,
    HTRLeaveSliceFinishCoord, DR_FINISH_LS_SIZE, DR_FINISH_SIZE, FR_FINISH_SIZE, HTR_FINISH_SIZE,
    HTR_LEAVE_SLICE_FINISH_SIZE,
};
...
pub static HTR_LEAVE_SLICE_FINISH_TABLE: LazyLock<HTRLeaveSliceFinishPruningTable> =
    LazyLock::new(|| gen_htr_ls_finish());
pub type HTRLeaveSliceFinishPruningTable =
    Box<dyn DepthEstimate<{ HTR_LEAVE_SLICE_FINISH_SIZE }, HTRLeaveSliceFinishCoord>>;
...
pub static DR_LEAVE_SLICE_FINISH_TABLE: LazyLock<DRLeaveSliceFinishPruningTable> =
    LazyLock::new(|| gen_dr_leave_slice_finish());
pub type DRLeaveSliceFinishPruningTable =
    Box<dyn DepthEstimate<{ DR_FINISH_LS_SIZE }, DRLeaveSliceFinishCoord>>;
...
const FINISH_FRUD_ST_MOVES: &[Turn333] = &[Turn333::F2, Turn333::B2, Turn333::L2, Turn333::R2];
...
pub const FINISH_FRUD_MOVESET: MoveSet =
    MoveSet::new_qt_ht_ordered(FINISH_FRUD_ST_MOVES, FINISH_AUX_MOVES);
pub const FINISH_HTR_MOVESET: MoveSet =
    MoveSet::new_qt_ht_ordered(FINISH_HTR_ST_MOVES, FINISH_AUX_MOVES);
...
impl DRFinishStep {
    ...
    pub(crate) fn get_possible_cancellation_count(
        solution: &Solution,
        dr_fin_axis: CubeAxis,
    ) -> usize {
        ...
        if let Some(x) = solution.steps.last() {
            ...
            let candidate = solution
                ...
                .filter(|x| {
                    (solution.ends_on_normal() && !x.alg.inverse_moves.is_empty())
            ...
            if let Some(canditate) = candidate.cloned() {
                ...
                candidate_cancel_moves
                    .iter()
                    .rev()
                    .take_while(|x| x.face.is_on_axis(dr_fin_axis))
                    .map(|x| if x.dir == Direction::Half { 1 } else { 2 })
                    .sum()
            } else {
                0
            }
...
mod test {
    ...
    fn test_dr_cancel_moves() {
        ...
        assert_eq!(
            0,
            DRFinishStep::get_possible_cancellation_count(&solution, CubeAxis::FB)
        );
        assert_eq!(
            1,
            DRFinishStep::get_possible_cancellation_count(&solution, CubeAxis::LR)
        );
        assert_eq!(
            0,
            DRFinishStep::get_possible_cancellation_count(&solution, CubeAxis::UD)
        );
    }
...
impl HTRFinishStep {
    pub fn new(dfs: DFSParameters, leave_slice: bool) -> StepGroup {
        debug!("Step fin with options {dfs:?}");
        if leave_slice {
            let variants = [CubeAxis::UD, CubeAxis::LR, CubeAxis::FB]
                .into_iter()
...
fn gen_fr_finish() -> FRFinishPruningTable {
    Box::new(
        InMemoryIndexTable::load_and_save("frfin", || {
...
fn gen_htr_finish() -> HTRFinishPruningTable {
    Box::new(
        InMemoryIndexTable::load_and_save("htrfin", || {
            lookup_table::generate(
                &HTR_FINISH_MOVESET,
...
fn gen_htr_ls_finish() -> HTRLeaveSliceFinishPruningTable {
    Box::new(
        InMemoryIndexTable::load_and_save("htrfinls", || {
            lookup_table::generate(
                &HTR_FINISH_MOVESET,
...
fn gen_dr_finish() -> DRFinishPruningTable {
    Box::new(
        MemoryMappedIndexTable::load_and_save("drfin", || {
            lookup_table::generate_large_table(&HTR_DR_UD_MOVESET)
        })
        .0,
    )
}
...
fn gen_dr_leave_slice_finish() -> DRLeaveSliceFinishPruningTable {
    Box::new(
        MemoryMappedIndexTable::load_and_save("drfinls", || {
            lookup_table::generate_large_table(&HTR_DR_UD_MOVESET)
        })
        .0,
    )
...
pub mod builder {
    ...
    impl<const A: bool, const B: bool, const C: bool, const D: bool>
        FRFinishBuilderInternal<A, B, C, D>
    {
        fn convert<const _A: bool, const _B: bool, const _C: bool, const _D: bool>(
            self,
        ) -> FRFinishBuilderInternal<_A, _B, _C, _D> {
    ...
    impl<const A: bool, const C: bool, const D: bool> FRFinishBuilderInternal<A, false, C, D> {
        ...
        ) -> FRFinishBuilderInternal<A, true, C, D> {
    ...
    impl<const A: bool, const B: bool, const C: bool> HTRFinishBuilderInternal<A, B, C> {
        fn convert<const _A: bool, const _B: bool, const _C: bool>(
            self,
        ) -> HTRFinishBuilderInternal<_A, _B, _C> {
    ...
    impl<const A: bool, const C: bool> HTRFinishBuilderInternal<A, false, C> {
        pub fn max_absolute_length(
            mut self,
            max_absolute_length: usize,
        ) -> HTRFinishBuilderInternal<A, true, C> {
            self._b_max_absolute_length = max_absolute_length;
            self.convert()
        }
    }
    ...
    impl<const A: bool, const B: bool, const C: bool, const D: bool>
        DRFinishBuilderInternal<A, B, C, D>
    {
        fn convert<const _A: bool, const _B: bool, const _C: bool, const _D: bool>(
            self,
        ) -> DRFinishBuilderInternal<_A, _B, _C, _D> {
    ...
    impl<const A: bool, const C: bool, const D: bool> DRFinishBuilderInternal<A, false, C, D> {
        pub fn max_absolute_length(
            mut self,
            max_absolute_length: usize,
        ) -> DRFinishBuilderInternal<A, true, C, D> {
            self._b_max_absolute_length = max_absolute_length;
            self.convert()
        }
    }
