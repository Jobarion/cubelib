...
pub type FRLeaveSlicePruningTableStep<'a> = DefaultPruningTableStep<
    'a,
    { FRUD_NO_SLICE_SIZE },
    FRUDNoSliceCoord,
    { HTRDRUD_SIZE },
    HTRDRUDCoord,
>;
pub type FRPruningTableStep<'a> = DefaultPruningTableStep<
    'a,
...
pub fn from_step_config_no_slice(
    table: &FRLeaveSlicePruningTable,
    config: StepConfig,
) -> Result<(Step333, DefaultStepOptions), String> {
    let step = if let Some(substeps) = config.substeps {
        let axis: Result<Vec<CubeAxis>, String> = substeps
    ...
    let step_variants = fr_axis
        ...
        .flat_map(move |x| {
            ...
            let x: Option<Box<dyn StepVariant + 'a>> = match x {
                CubeAxis::UD => Some(Box::new(FRLeaveSlicePruningTableStep::new(
                    &FR_UD_MOVESET,
                    vec![],
                    table,
                    Rc::new(vec![]),
                ...
                CubeAxis::FB => Some(Box::new(FRLeaveSlicePruningTableStep::new(
                    &FR_UD_MOVESET,
                ...
                CubeAxis::LR => Some(Box::new(FRLeaveSlicePruningTableStep::new(
                    &FR_UD_MOVESET,
...
pub fn fr<'a>(table: &'a FRPruningTable, fr_axis: Vec<CubeAxis>) -> Step333<'a> {
    let step_variants = fr_axis
        ...
        .flat_map(move |x| {
            ...
            let x: Option<Box<dyn StepVariant + 'a>> = match x {
                CubeAxis::UD => Some(Box::new(FRPruningTableStep::new(
                    &FR_UD_MOVESET,
                    vec![],
                    table,
                    Rc::new(vec![]),
                ...
                CubeAxis::FB => Some(Box::new(FRPruningTableStep::new(
                    &FR_UD_MOVESET,
                    vec![Transformation333::new(CubeAxis::X, Direction::Clockwise)],
                    table,
                    Rc::new(vec![]),
                ...
                CubeAxis::LR => Some(Box::new(FRPruningTableStep::new(
                    &FR_UD_MOVESET,
                    vec![Transformation333::new(CubeAxis::Z, Direction::Clockwise)],
                    table,
                    Rc::new(vec![]),
