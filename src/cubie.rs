...
impl EdgeCubieCube {
    ...
    unsafe fn unsafe_turn(&mut self, face: Face, turn_type: Turn) {
        self.0 = _mm_shuffle_epi8(self.0, Self::TURN_EDGE_SHUFFLE[face as usize][turn_type as usize]);
        if turn_type != Turn::Half {
            self.0 = _mm_xor_si128(self.0, Self::TURN_EO_FLIP[face as usize]);
        }
    }

    #[target_feature(enable = "avx2")]
    unsafe fn unsafe_transform(&mut self, axis: Axis, turn_type: Turn) {
        let edges_translated = _mm_shuffle_epi8(self.0, Self::TRANSFORMATION_EP_SHUFFLE[axis as usize][turn_type as usize]);
        ...
        let ep_translated = _mm_slli_epi32::<4>(_mm_shuffle_epi8(Self::TRANSFORMATION_EP_SHUFFLE[axis as usize][turn_type.invert() as usize], ep));
        let eo = if turn_type != Turn::Half {
            _mm_shuffle_epi8(Self::TRANSFORMATION_EO_MAP[axis], eo)
        } else {
            eo
        };
        self.0 = _mm_or_si128(ep_translated, eo);
    }
...
impl Turnable for EdgeCubieCube {
    ...
    #[inline]
    fn transform(&mut self, m: Transformation) {
        let Transformation(axis, turn) = m;
        unsafe {
            self.unsafe_transform(axis, turn);
        }
    }
}
...
impl CornerCubieCube {
    ...
    #[inline]
    unsafe fn unsafe_turn(&mut self, face: Face, turn_type: Turn) {
        self.0 = _mm_shuffle_epi8(self.0, Self::TURN_CORNER_SHUFFLE[face as usize][turn_type as usize]);
        if turn_type != Turn::Half {
            ...
            let corners_tmp = _mm_add_epi8(self.0, Self::TURN_CO_CHANGE[face as usize]);
            let overflow_bits = _mm_and_si128(corners_tmp, Self::CO_OVERFLOW_MASK);
            let not_overflow = _mm_srli_epi16::<2>(_mm_andnot_si128(corners_tmp, Self::CO_OVERFLOW_MASK));
    ...
    #[inline]
    unsafe fn unsafe_transform(&mut self, axis: Axis, turn_type: Turn) {
        let corners_translated = _mm_shuffle_epi8(self.0, Self::TRANSFORMATION_CP_SHUFFLE[axis as usize][turn_type as usize]);
        ...
        let cp_translated = _mm_slli_epi32::<5>(_mm_shuffle_epi8(Self::TRANSFORMATION_CP_SHUFFLE[axis as usize][turn_type.invert() as usize], cp));
        let co = if turn_type != Turn::Half {
            ...
            let co_id = _mm_or_si128(co_id, _mm_set_epi8(0, 0, 0, 0, 0, 0, 0, 0, 0b1000, 0, 0b1000, 0, 0b1000, 0, 0b1000, 0));
            _mm_shuffle_epi8(Self::TRANSFORMATION_CO_MAP[axis], co_id)
        } else {
            co
        };
        self.0 = _mm_or_si128(cp_translated, co);
    }
...
impl CubieCube {
    ...
    #[inline]
    unsafe fn unsafe_transform(&mut self, axis: Axis, turn_type: Turn) {
        self.edges.unsafe_transform(axis, turn_type);
        self.corners.unsafe_transform(axis, turn_type);
    }
